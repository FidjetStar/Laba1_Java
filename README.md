...
1) Проверить y == 0 → если да, вернуть 0.
...
4) В каждом случае вернуть строку вида: число + слово.
...
1) Создать переменную res = 1.
2) Запустить цикл for от i = 0 до i < y.
3) На каждой итерации умножить res на x.
...
3) Во внутреннем цикле печатать символ "*" без перевода строки.
4) После завершения внутреннего цикла перевести курсор на новую строку.
...
3) Скопировать элементы ins в позицию pos.
...
1) Запустить цикл for от i = 0 до i < arr.length / 2.
...
2) Скопировать все элементы arr1 в result.
3) Скопировать все элементы arr2 в result, начиная с позиции arr1.length.
...
1) Первый проход: подсчитать количество вхождений числа x в массив.
